...
<confirmation-modal :error="null" :show="showModalConfirmation" :title="modalTitle" :massiveDelete="massiveDelete" @confirm="deleteAccion" @confirmMassive="confirmMassiveDelete" @cancel="closeModal"></confirmation-modal>
<modal :show="!!message" :title="message" @close="handleError" :error="error" :loading="loading"></modal>
<section id="main">
    ...
    >
      {{ displayButtonText }}
          ...
          -->
    <button v-if="tratamientoInfoJson.porcentajeAvance == 100 && !esAltaGerencia" class="action-button" style="top: 140px" @click="register()">
        Generar informe
    </button>
    <div class="grid-container">
        <div class="grid-item">
            ...
            <input v-if="(update || !$route.params.tr_codigo) && canEdit" type="text" disabled="true" :placeholder="tratamientoInfoJson.codigo" />
        </div>
        <div class="grid-item">
            ...
            <input v-if="(update || !$route.params.tr_codigo) && canEdit" type="text" placeholder="Buscar por código" @keyup="searchCodigoEvaluacion" v-model.trim="codigoEvaluacion" />
            <div style="position: absolute">
                <p class="dropdownSelect" v-for="evaluacion in evaluacionList" :key="evaluacion.evaluacionId" @click="selectEvaluacion(evaluacion)">
        ...
        <div class="grid-item">
            ...
            <input v-if="(update || !$route.params.tr_codigo) && canEdit" type="text" placeholder="Plan de tratamiento-nombre" v-model.trim="tratamientoInfoJson.nombre" />
        ...
        <div class="grid-item">
            ...
            <select v-if="(update || !$route.params.tr_codigo) && canEdit" class="select" v-model="tratamientoInfoJson.usuarioId" name="analista">
                <option v-for="analista in analistaList" :key="analista.usuarioId" :value="analista.usuarioId">
                    {{
              analista.codigo +
              "-" +
              analista.nombres +
    ...
    <section id="acciones-mitigacion" v-if="showAccionesMitigacion">
        ...
        <div v-if="update && canEdit" class="flex-row table-options">
            <div>
                <i class="fas fa-plus-circle" @click="goToTestSummary"></i>&nbsp;&nbsp;&nbsp;<span style="text-decoration: underline; cursor: pointer" @click="goToTestSummary">Añadir nueva acción</span>
            </div>
            <div>
            ...
            <tbody>
                <tr v-for="accion in accionesMitigacion" :key="accion.accionMitigacionId">
                    <td v-if="update && canEdit">
                        <input type="checkbox" :id="accion.accionMitigacionId" :value="accion.accionMitigacionId" v-model="checkedEntities" />
                    </td>
                  ...
                  accion.descripcion
                            ...
                            <input v-if="update && canEdit" type="text" style="width: 90%; margin-left: 5px" v-model="accion.descripcion" />
                ...
                accion.fechaInicio
                  ...
                  : new Date().toISOString().substr(0, 10)
                        ...
                        <input v-if="update" type="date" @change="(e) => (accion.fechaInicio = e.target.value)" :value="
                ...
                accion.fechaFin
                  ...
                  : new Date().toISOString().substr(0, 10)
                        ...
                        <input v-if="update" type="date" @change="(e) => (accion.fechaFin = e.target.value)" :value="
...
import {
    IAccionMitigacion
} from "../interfaces/accionMitigacion";
import {
    BASE_URL,
    rol
